...
A *protection domain* can contain one or more data elements (such as global variables or arrays - this is known as a 'symbol' domain) or encompass an address range, such as an MMIO peripheral region (this is known as an 'address' domain).
...
- Optionally, enable extra debugging information (`mpu_verbose_faults`)
...
Memory protected systems *must* be compiled with a special set of flags, so ensure that the *correct build scripts are used* by the system to avoid linker errors when memory protection is enabled.
