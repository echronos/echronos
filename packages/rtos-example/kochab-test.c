...
{
    uint8_t count;
    rtos_signal_send_set(TASK_ID_A, 0);
    rtos_signal_send_set(TASK_ID_B, 0);

    debug_println("task a: taking lock");
    rtos_mutex_lock(MUTEX_ID_M0);
    rtos_yield();
    if (rtos_mutex_try_lock(MUTEX_ID_M0))
    {
        debug_println("unexpected mutex not locked.");
    }
    debug_println("task a: releasing lock");
    rtos_mutex_unlock(MUTEX_ID_M0);
    rtos_yield();
    ...
    for (count = 0; count < 10; count++)
    {
        ...
        if (count % 5 == 0)
        {
            ...
            rtos_signal_send_set(TASK_ID_B, 1);
        }
