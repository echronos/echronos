...
class Node:
    def __init__(self, caps, deps):
        self.caps = caps
        self.deps = deps

    @staticmethod
    def sort(nodes, fail_if_cyclic=True):
        """Sort nodes topologically, i.e., by their dependencies so that every item X that depends on item Y resides
        behind Y.

        Another interpretation of this algorithm is to convert a node/edge representation of a graph into a flat list
        of the nodes such that the leaf nodes are at the beginning and the root node(s) at the end of the list.

        If there are circular dependencies (i.e., if the graph is not acyclic) or a dependency is not defined as a
        capability (i.e., if any node referenced in an edge is not defined), an exception is raised.

        nodes: an iterable of Node objects

        Returns an iterable over the sorted Node objects.

        Example:
            zero = Node(set((0,)), set())
            one = Node(set((1,)), set((0,)))
            two = Node(set((2,)), set((0, 1)))
            sort((two, one, zero)) -> (zero, one, two)
        ...
        while todo:
            ...
            for node in todo:
                if node.deps.issubset(done_caps):
                    yield node
                    done_caps.update(node.caps)
                else:
                    new_todo.append(node)

            if len(new_todo) == len(todo):
                caps, deps = Node.get_all_caps_deps(nodes)
                unmet_dependencies = deps - caps
                if unmet_dependencies:
                    raise MissingDependencies('Topologic sorting failed; the dependencies {} of the nodes {} are not \
met by the capabilities {} of other nodes'.format(unmet_dependencies, todo, caps))
                else:
                    if fail_if_cyclic:
                        raise CyclicDependencies('Topologic sorting failed; the following nodes contain cyclic \
dependencies: {}'.format(todo))
                    else:
                        # Return remaining nodes in any order as they are known to resolve each others' dependencies
                        # This may lead to suboptimal results where the cyclic dependencies exist only between some
                        # of the remaining nodes.
                        # In such a case, the other remaining nodes could still be topologically sorted, but the
                        # implementation here does not address that case.
                        for node in todo:
                            yield node
                        new_todo = ()
    ...
    @staticmethod
    def get_all_caps_deps(nodes):
        caps = set()
        deps = set()
        for node in nodes:
            caps.update(node.caps)
...
class CyclicDependencies(TopologicalSortError):
    pass


class MissingDependencies(TopologicalSortError):
    pass
