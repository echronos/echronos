...
IF NOT EXIST C:\splint-3.1.2 (
    powershell.exe -nologo -noprofile -command "& { Invoke-WebRequest 'http://ssrg.nicta.com.au/Downloads/echronos_regression_binaries/splint-3.1.2.win32.zip' -OutFile 'splint-3.1.2.win32.zip'; Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('splint-3.1.2.win32.zip', 'C:\'); }" || EXIT /B %ERRORLEVEL%
